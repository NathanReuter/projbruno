...
for (index in 1:nrow(planilha11)) {
  ...
  if (nrow(PC) > 0) {
    PCVector <- c(PCVector, PC$Porcentagem.Conselheiros[1]);
  } else {
  ...
  if (nrow(POE) > 0) {
    POEDIRECTOR <- c(POEDIRECTOR, POE$Diretor[1]);
    POEPercent <- c(POEPercent, POE$Percentual.de.Ações[1]);
  } else {
  ...
  if (nrow(AM) > 0) {
    AMDirector <- c(AMDirector, AM$Diretor[1]);
  ...
  if (nrow(SIZE) > 0) {
    SizeVector <- c(SizeVector, SIZE$Valor.Ativo[1]);
  } else {
    SizeVector <- c(SizeVector, 0);
  }
  
  # SET <- filter(planilha12, planilha12$Codigo == actualCode & planilha12$Ano == actualYear )
  # 
  # if (nrow(SET) > 0) {
  #   SETVector <- c(SETVector, SET$Valor.Ativo[1]);
  # } else {
  #   SETVector <- c(SETVector, 0);
  # }
  
}

planilhaCompleta <- planilha11;
colnames(planilhaCompleta)[4] = "NIV"
...
planilhaCompleta["POEDIRECTOR"] = POEDIRECTOR;
...
write.csv(planilhaCompleta, "./completas/PlanilhaCompleta.csv");